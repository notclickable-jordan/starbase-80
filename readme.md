...
| BGCOLOR                  |          | theme(colors.slate.50) | Page background color in light mode. Accepts a hex color (e.g. `#f8fafc`) or a Tailwind color as `var(--color-slate-50)`. Legacy `theme(colors.slate.50)` syntax is still accepted and auto-converted. |
| BGCOLORDARK              |          | theme(colors.gray.950) | Page background color in dark mode. Accepts a hex color or a Tailwind color as `var(--color-gray-950)`. Legacy `theme(colors.gray.950)` syntax is still accepted and auto-converted.                    |
| CATEGORYBUBBLECOLORLIGHT |          | theme(colors.white)    | Background color for category bubbles (if enabled) in light mode. Accepts a hex color or a Tailwind color as `var(--color-white)`. Legacy `theme(colors.white)` syntax is still accepted.               |
| CATEGORYBUBBLECOLORDARK  |          | theme(colors.black)    | Background color for category bubbles (if enabled) in dark mode. Accepts a hex color or a Tailwind color as `var(--color-black)`. Legacy `theme(colors.black)` syntax is still accepted.                |
